-- ============================================================
-- Tech With Hussain — Local Dev Schema (XAMPP compatible)
-- ============================================================
...
INSERT IGNORE INTO `site_settings` (`setting_key`, `setting_val`) VALUES
  ...
  ('google_analytics_id',  ''),
  ('maintenance_mode',     '0'),
  ('custom_cursor_public', '1'),
  ('custom_cursor_admin',  '1');
...
INSERT IGNORE INTO `projects` (`title`, `slug`, `excerpt`, `category`, `technologies`, `thumbnail`, `live_url`, `is_featured`, `sort_order`, `status`) VALUES
...
INSERT IGNORE INTO `blog_posts` (`title`, `slug`, `excerpt`, `category`, `tags`, `author_id`, `is_featured`, `status`, `published_at`) VALUES
  ('Mastering n8n for Workflow Automation', 'mastering-n8n-automation', 'How to connect OpenAI, Gmail, and Slack to build automation workflows.', 'AI & Tech', '["n8n","OpenAI","Automation"]', 1, 1, 'published', NOW()),
  ('How I Fixed Core Web Vitals on WordPress', 'fixing-wordpress-core-web-vitals', 'A case study on optimizing TTFB, CLS, and LCP to pass Google standards.', 'Web Dev', '["WordPress","SEO","Performance"]', 1, 1, 'published', NOW()),
  ('The Local SEO Guide to Ranking #1 on Google Maps', 'local-seo-ranking-google-maps', 'Discover the exact strategies to dominate local search.', 'SEO Marketing', '["SEO","Local SEO","Schema"]', 1, 0, 'published', NOW()),
  ('Why You Should Stop Storing JWT in localStorage', 'secure-jwt-cookies-php', 'An investigation into XSS vulnerability risks with localStorage JWT.', 'Web Dev', '["Security","JWT","PHP"]', 1, 0, 'published', NOW());
...
INSERT IGNORE INTO `testimonials` (`name`, `designation`, `company`, `content`, `rating`, `is_featured`, `sort_order`) VALUES
  ('Ravi Sharma',    'E-commerce Director', 'WalnutWala',        'Hussain ranked us #1 on Google in 4 months. Organic traffic increased by 340%.', 5, 1, 1),
  ('Sarah Chen',     'CEO',                 'NovaTech Solutions','The AI chatbot Hussain built tripled our lead conversions. Code quality was exceptional.', 5, 1, 2),
  ('Ahmed Al-Farsi', 'Marketing Head',      'Dubai Exports LLC', 'Website went from 45 to 98 on PageSpeed. Bounce rate dropped by 60%.', 5, 1, 3),
  ('Priya Nair',     'Co-Founder',          'BrandBloom India',  'Full rebranding and web development done in 3 weeks. Design is stunning.', 5, 0, 4);
...
INSERT IGNORE INTO `experience` (`company`, `role`, `type`, `location`, `start_date`, `is_current`, `sort_order`) VALUES
...
INSERT IGNORE INTO `certifications` (`title`, `issuer`, `issued_date`, `category`, `sort_order`) VALUES
...
CREATE TABLE IF NOT EXISTS `skills` (
  ...
  `icon`         VARCHAR(100)   DEFAULT NULL,
  `level`        TINYINT        NOT NULL DEFAULT 90,
  `sort_order`   INT            NOT NULL DEFAULT 0,
  `status`       ENUM('active','inactive') NOT NULL DEFAULT 'active',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
